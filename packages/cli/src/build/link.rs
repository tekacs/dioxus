...
impl BuildRequest {
    ...
    pub(crate) fn rewrite_workspace_dep_externs(
        ...
        for dependency in self.workspace_dependencies_of(crate_name) {
            let dep_args = workspace_rustc_args
                .rustc_args
    ...
    fn find_workspace_dep_artifact(
        ...
        if preferred_ext == Some("rlib") {
            if let Some(rlib) =
                rustc_args.and_then(|args| self.find_rlib_for_crate(crate_name, args).ok())
            {
                return Some(rlib);
            }
            if let Some(rlib) =
        ...
        rustc_args
            .and_then(|args| self.find_rlib_for_crate(crate_name, args).ok())
